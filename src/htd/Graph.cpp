...
htd::Graph::Graph(const htd::LibraryInstance* const manager)
	: base_(manager->hypergraphFactory().createInstance()) { }

htd::Graph::Graph(const htd::LibraryInstance* const manager, std::size_t initialSize)
	: base_(manager->hypergraphFactory().createInstance(initialSize)) { }
...
htd::Graph::Graph(const htd::IGraph& original)
	: base_(original.managementInstance()->hypergraphFactory().createInstance(original)) { }
...
std::size_t htd::Graph::vertexCount(void) const { return base_->vertexCount(); }

std::size_t htd::Graph::edgeCount(void) const { return base_->edgeCount(); }

std::size_t htd::Graph::edgeCount(htd::vertex_t vertex) const { return base_->edgeCount(vertex); }

bool htd::Graph::isVertex(htd::vertex_t vertex) const { return base_->isVertex(vertex); }

bool htd::Graph::isEdge(htd::id_t edgeId) const { return base_->isEdge(edgeId); }
...
htd::ConstCollection<htd::id_t> htd::Graph::associatedEdgeIds(htd::vertex_t vertex1,
		htd::vertex_t vertex2) const {
	return base_->associatedEdgeIds(vertex1, vertex2);
}

htd::ConstCollection<htd::id_t> htd::Graph::associatedEdgeIds(
		const std::vector<htd::vertex_t>& elements) const {
	return base_->associatedEdgeIds(elements);
}

htd::ConstCollection<htd::id_t> htd::Graph::associatedEdgeIds(
		const htd::ConstCollection<htd::vertex_t>& elements) const {
	return base_->associatedEdgeIds(elements);
}
...
bool htd::Graph::isConnected(void) const { return base_->isConnected(); }
...
htd::ConstCollection<htd::vertex_t> htd::Graph::vertices(void) const { return base_->vertices(); }
...
std::size_t htd::Graph::isolatedVertexCount(void) const { return base_->isolatedVertexCount(); }
...
htd::FilteredHyperedgeCollection htd::Graph::hyperedgesAtPositions(
		const std::vector<htd::index_t>& indices) const {
	return base_->hyperedgesAtPositions(indices);
}

htd::FilteredHyperedgeCollection htd::Graph::hyperedgesAtPositions(
		std::vector<htd::index_t>&& indices) const {
	return base_->hyperedgesAtPositions(std::move(indices));
}

htd::vertex_t htd::Graph::nextVertex(void) const { return base_->nextVertex(); }

htd::id_t htd::Graph::nextEdgeId(void) const { return base_->nextEdgeId(); }

htd::vertex_t htd::Graph::addVertex(void) { return base_->addVertex(); }

htd::vertex_t htd::Graph::addVertices(std::size_t count) { return base_->addVertices(count); }
...
void htd::Graph::removeEdge(htd::id_t edgeId) { base_->removeEdge(edgeId); }
...
htd::Graph* htd::Graph::clone(void) const { return new htd::Graph(*this); }

#ifdef HTD_USE_VISUAL_STUDIO_COMPATIBILITY_MODE
htd::IGraph* htd::Graph::cloneGraph(void) const { return clone(); }

htd::IMultiGraph* htd::Graph::cloneMultiGraph(void) const { return clone(); }

htd::IHypergraph* htd::Graph::cloneHypergraph(void) const { return clone(); }

htd::IGraphStructure* htd::Graph::cloneGraphStructure(void) const { return clone(); }

htd::IMultiHypergraph* htd::Graph::cloneMultiHypergraph(void) const { return clone(); }

htd::IMutableGraph* htd::Graph::cloneMutableGraph(void) const { return clone(); }
#endif
...
htd::Graph& htd::Graph::operator=(const htd::IGraph& original) {
	if (this != &original) {
		htd::IMutableHypergraph* newBase =
				managementInstance()->hypergraphFactory().createInstance(original);
...
#ifdef HTD_USE_VISUAL_STUDIO_COMPATIBILITY_MODE
void htd::Graph::assign(const htd::IGraph& original) { *this = original; }
#endif
