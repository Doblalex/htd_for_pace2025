...
htd::Hypergraph::Hypergraph(const htd::LibraryInstance* const manager)
	: base_(manager->multiHypergraphFactory().createInstance()) { }

htd::Hypergraph::Hypergraph(const htd::LibraryInstance* const manager, std::size_t initialSize)
	: base_(manager->multiHypergraphFactory().createInstance(initialSize)) { }
...
htd::Hypergraph::Hypergraph(const htd::IHypergraph& original)
	: base_(original.managementInstance()->multiHypergraphFactory().createInstance(original)) {
	*this = original;
}
...
std::size_t htd::Hypergraph::vertexCount(void) const { return base_->vertexCount(); }

std::size_t htd::Hypergraph::edgeCount(void) const { return base_->edgeCount(); }

std::size_t htd::Hypergraph::edgeCount(htd::vertex_t vertex) const {
	return base_->edgeCount(vertex);
}

bool htd::Hypergraph::isVertex(htd::vertex_t vertex) const { return base_->isVertex(vertex); }

bool htd::Hypergraph::isEdge(htd::id_t edgeId) const { return base_->isEdge(edgeId); }
...
htd::ConstCollection<htd::id_t> htd::Hypergraph::associatedEdgeIds(htd::vertex_t vertex1,
		htd::vertex_t vertex2) const {
	return base_->associatedEdgeIds(vertex1, vertex2);
}

htd::ConstCollection<htd::id_t> htd::Hypergraph::associatedEdgeIds(
...
bool htd::Hypergraph::isConnected(void) const { return base_->isConnected(); }
...
htd::FilteredHyperedgeCollection htd::Hypergraph::hyperedgesAtPositions(
...
const htd::Hyperedge& htd::Hypergraph::hyperedgeAtPosition(htd::index_t index,
		htd::vertex_t vertex) const {
	return base_->hyperedgeAtPosition(index, vertex);
}

htd::vertex_t htd::Hypergraph::nextVertex(void) const { return base_->nextVertex(); }

htd::id_t htd::Hypergraph::nextEdgeId(void) const { return base_->nextEdgeId(); }

htd::vertex_t htd::Hypergraph::addVertex(void) { return base_->addVertex(); }

htd::vertex_t htd::Hypergraph::addVertices(std::size_t count) { return base_->addVertices(count); }
...
void htd::Hypergraph::removeEdge(htd::id_t edgeId) { base_->removeEdge(edgeId); }
...
htd::Hypergraph* htd::Hypergraph::clone(void) const { return new htd::Hypergraph(*this); }

#ifdef HTD_USE_VISUAL_STUDIO_COMPATIBILITY_MODE
htd::IHypergraph* htd::Hypergraph::cloneHypergraph(void) const { return clone(); }

htd::IGraphStructure* htd::Hypergraph::cloneGraphStructure(void) const { return clone(); }

htd::IMultiHypergraph* htd::Hypergraph::cloneMultiHypergraph(void) const { return clone(); }

htd::IMutableHypergraph* htd::Hypergraph::cloneMutableHypergraph(void) const { return clone(); }
#endif
...
htd::Hypergraph& htd::Hypergraph::operator=(const htd::IHypergraph& original) {
	if (this != &original) {
		htd::IMutableMultiHypergraph* newBase =
				managementInstance()->multiHypergraphFactory().createInstance(original);
...
#ifdef HTD_USE_VISUAL_STUDIO_COMPATIBILITY_MODE
void htd::Hypergraph::assign(const htd::IHypergraph& original) { *this = original; }
#endif
